...
;;	RETI
	...
	;; Wait to exit until STAT mode 0 or 1 to avoid
	;; returning in mode 2 or 3 and potentially 
	;; resuming code that is not allowed to write to VRAM
	;; during those modes
3$:	LDH	A,(.STAT)
	AND 	#0x02
	JR	NZ, 3$
...
4$:
	...
	;; LCD ISR shouldnt gate exit on STAT mode like the other
.int_LCD_no_exit_wait:
	PUSH	AF
	PUSH	BC
	PUSH	DE
1$:
...
; Moved to below to below
; See 
; inaccessible vram exceptions in BGB at startup in cart troubleshooting mode
; https://github.com/Zal0/gbdk-2020/pull/24/commits/89b89dc0ebcfa31a51a0b0fe6dcd582209ccea8a
;
...
;   DEC DE
;   INC HL
    ; Fix for _remove_VBL() erasing previous VBL entry
    ; Copy next entry -> current entry, instead of copying current -> previous
    LD  D,H
    LD  E,L
    INC HL
    INC HL
